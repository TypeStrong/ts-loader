...
function loader(this: interfaces.Webpack, contents: string) {
    ...
    const { outputText, sourceMapText } = options.transpileOnly
        ...
        : getEmit(rawFilePath, filePath, instance, generatedFiles, this);
...
function getEmit(
    ...
    const allDefinitionFiles = Object.keys(instance.files)
        .filter(defFilePath => !!defFilePath.match(constants.dtsDtsxRegex))
        .filter(filePath => !generatedFiles.hasOwnProperty(filePath));
    ...
    */
    ...
    if (additionalDependencies) {
        additionalDependencies.filter(filePath => !generatedFiles.hasOwnProperty(filePath)).forEach(addDependency);
    }
