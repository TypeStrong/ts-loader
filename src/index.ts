...
function loader(this: interfaces.Webpack, contents: string) {
    ...
    const usePreviousLoaderGeneratedFiles = hasGeneratedFiles(this);
    let generatedFiles: { [filePath: string]: string };
    if (usePreviousLoaderGeneratedFiles) {
        // Load files generated by previous loaders in the same chain
        generatedFiles = getGeneratedFiles(this);
        for (let filePath of Object.keys(generatedFiles)) {
            updateFileInCache(filePath, generatedFiles[filePath], instance);
        }
    }
    ...
    if (usePreviousLoaderGeneratedFiles) {
        // Remove dependencies on files generated by previous loaders in the same chain.
        removeDependencies(this, filePath => generatedFiles[filePath] !== undefined);
    }
...
/**
 * Returns true if there are any files generated by previous loaders in the same chain.
 */
function hasGeneratedFiles(loader: interfaces.Webpack): boolean {
    for (let i = loader.loaders.length - 1; i > loader.loaderIndex; i--) {
        const data = loader.loaders[i].data;
        const files = data && data['ts-loader-files'];
...
/**
 * Get any files generated by previous loaders in the same chain.
 */
function getGeneratedFiles(loader: interfaces.Webpack) {
    ...
    for (let i = loader.loaders.length - 1; i > loader.loaderIndex; i--) {
        ...
        if (files) {
            for (let filePath of Object.keys(files)) {
                if (filePath) {
                    generatedFiles[path.normalize(filePath)] = files[filePath];
                }
            }
        }
    }
...
function getEmit(
    ...
    removeDependencies(loader, filePath => filePath === loader.resourcePath);
