/// <reference path="D.d.ts" />
/// <reference path="K.d.ts" />
/// <reference path="F.d.ts" />
import module_de0 = require("./de")
import module_bz1 = require("./bz")
import module_cw2 = require("./cw")
import module_dq3 = require("./dq")
import module_bq4 = require("./bq")
import module_cf5 = require("./cf")
import module_co6 = require("./co")
import module_dl7 = require("./dl")
import module_cn8 = require("./cn")
import module_bq9 = require("./bq")

D.doSomething()
K.doSomething()
F.doSomething()
module_de0.doSomething()
module_bz1.doSomething()
module_cw2.doSomething()
module_dq3.doSomething()
module_bq4.doSomething()
module_cf5.doSomething()
module_co6.doSomething()
module_dl7.doSomething()
module_cn8.doSomething()
module_bq9.doSomething()
export function doSomething() { }
